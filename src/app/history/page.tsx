...
import { getHistory, deleteHistoryEntry, clearHistory, HistoryEntry } from '@/lib/history';
...
export default function HistoryPage() {
  ...
  const filtered = entries.filter(e => {
    ...
    const matchesFilter = filter === 'all' || e.marketplace === filter || e.marketplace === 'both';
  ...
  const downloadCSV = (entry: HistoryEntry) => {
    ...
    saveAs(blob, `${entry.fileName.replace('.pdf', '')}_shopify.csv`);
  };

  const downloadAmazon = async (entry: HistoryEntry) => {
    ...
    saveAs(new Blob([buffer]), `${entry.fileName.replace('.pdf', '')}_amazon.xlsx`);
  };
              ...
              >
                ...
                >
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0" onClick={e => e.stopPropagation()}>
                    {(entry.marketplace === 'shopify' || entry.marketplace === 'both') && (
                      <button
                    ...
                    {(entry.marketplace === 'amazon' || entry.marketplace === 'both') && (
                      <button
                  ...
                  {expandedId === entry.id && (
                    ...
                    >
                      <div className="border-t border-slate-100 bg-slate-50 px-5 py-4">
                        ...
                        <p className="text-xs text-slate-400 mt-3">Processed: {formatDate(entry.timestamp)}</p>
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
